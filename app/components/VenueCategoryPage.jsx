...
export default function VenueCategoryPage({
    ...
    const fetchHalls = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${getApiBaseUrl()}/api/halls/public${queryString ? `?${queryString}` : ""}`
        );
  ...
  const filteredHalls = useMemo(() => {
    return baseFilteredHalls.filter(
      ...
      <div className="page-hero">
        ...
        <p className="page-subtitle">
          {displayLocationLabel
            ? `Showing registered ${title} venues in ${displayLocationLabel}`
            : `Showing all registered ${title} venues`}
        </p>
      </div>
      ...
      <div className="wedding-layout">
        ...
        <div className="hall-card-grid">
          {!loading && filteredHalls.length === 0 && (
            <p style={{ color: "#777" }}>No venues available under this category</p>
          )}
